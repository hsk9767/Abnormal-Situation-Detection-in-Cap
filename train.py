...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
    ...
    for epochId in range(current_epoch, 280):
        scheduler.step()
        total_losses = [0, 0] * (num_refinement_stages)  # heatmaps loss, paf loss per stage
        ...
        for batch_data in train_loader:
            ...
            for loss_idx in range(len(total_losses) // 2):
                losses.append(l2_loss(stages_output[loss_idx], paf_maps, paf_masks, images.shape[0]))
                total_losses[loss_idx] += losses[-1].item() / batches_per_iter

            for loss_idx in range(len(total_losses) // 2):
                losses.append(l2_loss(stages_output[(len(total_losses)//2)+loss_idx], keypoint_maps, keypoint_masks, images.shape[0]))
                total_losses[(len(total_losses)//2)+loss_idx] += losses[-1].item() / batches_per_iter
            ...
            if num_iter % log_after == 0:
                ...
                for loss_idx in range(len(total_losses) // 2):
                    print('\n'.join(['stage{}_pafs_loss:     {}', 'stage{}_heatmaps_loss: {}']).format(
                        ...
                        loss_idx + 1, total_losses[(len(total_losses) // 2)+loss_idx] / log_after))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val-after', type=int, default=3000,
