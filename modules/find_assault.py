...
class Find_assault():
    def __init__(self, Extractor):
        ...
        self.weird_state_count_limit = 50
        self.weird_state_count = 0
        self.weird_state_flag = False
        self.cos_dist_threshold = 4.0
    ...
    def get_cosine_distance(self, a, b):
        ...
        return 100. - np.dot(a, b.T)*100

    def waring(self, weird_state_count):
        if weird_state_count > self.weird_state_count_limit:
            if self.weird_state_flag == False:
                self.weird_state_count += 50
                self.weird_state_flag = True
            return True
        else:
            False
    ...
    def find_driver(self, current_poses, img):
        ...
        tr_x_arr = np.array(list(map(lambda x  : x  if x > 2 * img.shape[1] / 3 else -1, tr_x_list)))
        if tr_x_arr.size > 0:
            most_right_index = np.argmax(tr_x_arr)
        else: ##driver 찾는 과정에서 아무런 오른쪽 부분에 아무런 사람이 존재하지 않으면 다시 찾기 시작해야 함.
            self.find_driver_count = 0
            return False , self.find_driver_count, "No one Detected" ## return 값은 driver_find 플래그에 대한 bool 값
        ...
        if self.find_driver_count == 0:
            ...
            return False, self.find_driver_count, "First Detect"
            ...
            if dist_from_center_avg > self.eyes_nose_dist_avg:
                self.find_driver_count = 0
                return False, self.find_driver_count, "Too Far Away"
                ...
                return False, self.find_driver_count, "Normal"
            else:
                return True, self.find_driver_count, "Found"
    ...
    def is_driver(self, current_poses, img):
        ...
        if tr_x_arr.size > 0:
            ...
            return False, -1, self.weird_state_count, self.waring(self.weird_state_count)
        ...
        if area < 0.5 * self.area_avg:
            self.weird_state_count += 1
            #print("Area 가 많이 작아진 경우")
            return False, -1, self.weird_state_count , self.waring(self.weird_state_count)
        elif dist_from_center_avg < self.eyes_nose_dist_avg * 1.1 :##check 할 때에는 좀 더 넓은 오차범위를 둠.
            self.weird_state_count = max(0, self.weird_state_count - 1)
            #print("Center Point 가 별 차이나지 않는 경우")
            return True, most_right_index, self.weird_state_count , self.waring(self.weird_state_count)
        else:
            tl_x, tl_y, width, height = current_poses[most_right_index].bbox
            tl_x = max(0, tl_x)
            tl_y = max(0, tl_y)
